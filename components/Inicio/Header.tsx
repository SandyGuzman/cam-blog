...
const Header: React.FC = () => {
  ...
  return (
    <header className="absolute top-0 left-0 sm:bottom sm:top-0  w-full flex sm:flex-none md:flex-row flex-col overflow-hidden justify-between items-center py-2 px-5 z-[50]">
      ...
      <div className="flex items-center md:flex-row">
        ...
        >
          {isMenuOpen ? (
            <span className="block w-8 h-8 text-white text-2xl font-bold">&times;</span>
          ) : (
            <>
        ...
        <div className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'} absolute top-0 left-0 w-full h-full bg-gray-800 bg-opacity-95 z-50`}>
          <nav className="flex flex-col items-center space-y-6 pt-20">
            <Link href="/" onClick={closeMenu}>
              <span className="text-[#95DE1C] font-semibold text-2xl">INICIO</span>
            </Link>
            <Link href="/noticias" onClick={closeMenu}>
              <span className="text-white font-semibold text-2xl">NOTICIAS</span>
            </Link>
            <button
